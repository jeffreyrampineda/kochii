...
@Component({
  selector: 'app-post-creations-edit',
...
export class PostCreationsEditComponent implements OnInit {
  ...
  createPostForm(postFormData?: Post): void {
    ...
    if (postFormData?.ingredients.length > 0) {
      postFormData?.ingredients.forEach(
        (ingredient: {
    ...
    if (postFormData?.instructions.length > 0) {
      postFormData?.instructions.forEach(
        (instruction: { description: string | undefined }) => {
  ...
  onDelete(): void {
    const post_id = this.postForm.get('_id')?.value;
    if (post_id) {
      this.recipesService.deletePost(post_id).subscribe({
        next: (result) => {
