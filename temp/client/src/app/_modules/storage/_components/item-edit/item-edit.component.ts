...
export class ItemEditComponent implements OnInit {
  ...
  groups: string[] = [];
  firstFood: any = {};
  similarFoods: any = []; // Foods with similar names,
  selectedSimilarFood: any = {
    foodNutrients: [],
  };
  ...
  addMoreForms(): void {
    this.itemEditForms.push(
      this.formBuilder.group({
        ...
        cost: [
          this.itemModel.cost,
          [Validators.min(0), Validators.max(999), Validators.required],
        ],
        quantity: [
  ...
  f(idx: number) {
    return this.itemEditForms[idx].controls;
  }
  ...
  openEditDialog(title: string, description: string): void {
    const dialogRef = this.dialog.open(UpdateDialogComponent, {
      ...
      data: {
        ...
        items: Object.values(this.itemEditForms).map((form: FormGroup) =>
          form.getRawValue()
        ),
